...
const Home: NextPage = () => {
  const [balancez, setBalancez] = useState<number | null>(null);
...
const  fetchBalance = async () => {
  try {
    ...
    if (response.ok && typeof data.result === 'string' && data.result !== '') {
      const balances = parseFloat(data.result) / 1e18;
      const finalBalance = balances.toFixed(2);
  ...
  const balances = fetchBalance();
  console.log(balances);
  const balancesString = balances.toString() as React.ReactNode
  ...
  useEffect(() => {
    fetchBalance().then((balancez) => {
      setBalancez(balancez);
    });
  }, []);
  ...
  return (
    ...
    >
      ...
      <p className="text-xl font-bold">Your balance is: {balanceContent}</p>
      <p className="text-lg">Your Cro Crook NFT Balance is: {balanceContent}</p>
    </div>*/}
        <div>
          <DashboardLayout balancez={balancez} balanceContent={balanceContent}/>
